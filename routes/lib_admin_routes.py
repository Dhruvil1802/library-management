...
from schemas.lib_admin_schemas import LibAdminBase,Token
...
@router.post("/lib_admin/registration", response_model=LibAdminBase, status_code=201)
def register_lib_admin(lib_admin: LibAdminBase, db: Session = Depends(get_db)):
    existing_lib_admin = lib_admin_crud.get_lib_admin_by_email(db, email=lib_admin.email)
    if existing_lib_admin:
        raise HTTPException(status_code=400, detail="Email already registered")
    return lib_admin_crud.create_lib_admin(db=db, lib_admin=lib_admin)  
